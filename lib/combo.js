...
var debug		= require('debug')('combo:combo');
			...
			async.eachLimit(self.files, self.eachFileLimit, function(file, callback)
			{
				...
				else
				{
					fs.stat(file, function(err, stat)
					{
						...
						mtimes.push(stat.mtime);
						sizes.push(stat.size);
						var cacheData = {mtime: stat.mtime, size: stat.size};
						fileStatCache.set(file, cacheData);
						callback();
					});
				}
			},
		...
		pro = pro.then(function()
			{
				return new Promise(function(resolve, reject)
					{
						fileRead(file, fileCache, self.maxCacheFileSize)
...
{
	var combo = new Combo(files, options);
	var st = new Readable(
		{
			read: function(){}
		});
