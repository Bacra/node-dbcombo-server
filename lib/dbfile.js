...
{
	if (!groups || !groups.length || !groups[groups.length-1]) throw new Error('NO FILE INDEX,'+indexStr);

	var requestMaxIndex = ClientKey.parse.maxIndexInGroup(groups);
	debug('[%s]global len:%d group:%o, max index:%d, file lenth:%d', indexStr, groups.length, groups, requestMaxIndex, filelist.length);
